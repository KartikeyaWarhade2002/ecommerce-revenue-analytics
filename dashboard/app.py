...
Please ensure analytics_orders.csv exists in:
...
selected_states = st.sidebar.multiselect(
    "Customer State",
    options=sorted(df_raw["customer_state"].dropna().unique()),
)

selected_categories = st.sidebar.multiselect(
    "Product Category",
    options=sorted(df_raw["product_category"].dropna().unique()),
)

selected_payments = st.sidebar.multiselect(
    "Payment Type",
    options=sorted(df_raw["payment_type"].dropna().unique()),
)

selected_statuses = st.sidebar.multiselect(
    "Order Status",
    options=sorted(df_raw["order_status"].dropna().unique()),
)
...
if df.empty:
    st.warning(
        "No data matches the selected filters. Please adjust your filter selections."
    )
    st.stop()
...
delivered_pct = (
    (orders_df["order_status"] == "delivered").mean()
    * 100
)
...
with tab_sales:
    ...
    inst = (
        orders_df["payment_installments"]
        .value_counts()
        .sort_index()
        .reset_index()
    )
...
with tab_products:
    ...
    st.subheader("Average Price vs Average Freight by Category (Top 15 by Volume)")
    ...
    pf = (
        df[df["product_category"].isin(top15)]
...
with tab_ops:
    ...
    with col2:
        ...
        delta = orders_df.dropna(
            subset=["delivery_delta_days"]
        ).copy()
    ...
    state_delivery = (
        orders_df.dropna(subset=["delivery_days"])
...
with tab_data:
    ...
    st.caption(
        f"{len(df):,} rows shown · "
        "flag_delivered_missing_date marks delivered orders "
        "that do not have a recorded delivery timestamp."
    )
